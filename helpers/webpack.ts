import {waitForExportBySource} from "./index.tsx";
...
function queryToFilter(query: Query | WaitQuery): ModuleFilter {
    ...
    if ("prototypeKeys" in query) return Webpack.Filters.byPrototypeKeys(...query.prototypeKeys) as any;
...
async function resolveModuleAsync<T = any>(filter: ModuleFilter, options?: WaitOptions): Promise<T | null> {
    const opts = options ?? {};
    if (opts.declaration) {
...
export function wpGetBySource<T = any>(source: Array<string | RegExp>, options?: GetOptions): T | null {
...
export function wpGetBulkKeyed<T extends Record<string, Query>>(queries: T): { [K in keyof T]: any } {
...
export function wpGetMangled<T extends Record<string, any>>(
    ...
    return Webpack.getMangled(filter as any, map as any, {...(options ?? {}), mapDeclarations: true}) as T;
}
...
export async function wpWait<T = any>(filter: ModuleFilter, options?: WaitOptions): Promise<T | null> {
...
export function wpWaitByPrototypeKeys<T = any>(keys: string[], options?: WaitOptions): Promise<T | null> {
    return resolveModuleAsync<T>(Webpack.Filters.byPrototypeKeys(...keys) as any, options);
}

export function wpWaitByStrings<T = any>(strings: string[], options?: WaitOptions): Promise<T | null> {
    return resolveModuleAsync<T>(Webpack.Filters.byStrings(...strings) as any, options);
}

export function wpWaitBySource<T = any>(source: Array<string | RegExp>, options?: WaitOptions): Promise<T | null> {
...
export function wpWaitByDisplayName<T = any>(displayName: string, options?: WaitOptions): Promise<T | null> {
    return resolveModuleAsync<T>(Webpack.Filters.byDisplayName(displayName) as any, options);
}

export function wpWaitForStore<T = any>(storeName: string, options?: WaitOptions): Promise<T | null> {
...
export function wpWaitBulk<T extends WaitQuery[]>(...queries: [...T]): Promise<{ [K in keyof T]: any }> {
...
export async function wpWaitMangled<T extends Record<string, any>>(
    ...
    const filter = typeof locator === "function"
        ? (locator as any)
        : (Webpack.Filters.bySource(locator as any) as any);
    await Webpack.waitForModule(filter, options ?? {});
    return Webpack.getMangled(locator as any, map as any, {...(options ?? {}), mapDeclarations: true}) as T;
}
...
export async function wpWaitDeclaration<T = any>(
    ...
    return resolveWaitQuery({...query, options: {...(query.options ?? {}), declaration: match}} as WaitQuery);
}

export function wpGetDeclaration<T = any>(query: Query, match: ModuleFilter): T | null {
    return resolveQuery({...query, options: {...(query.options ?? {}), declaration: match}} as Query);
}

export async function wpPatchOnce<M extends object, K extends Extract<keyof M, string>>(
    config: PatchOnceConfig<M, K>
): Promise<(() => void) | null> {
    const mod = (await Webpack.waitForModule(config.filter as any, config.waitOptions ?? {})) as M | undefined;
    if (!mod) return null;
    return runPatch(config.patcher, config.pluginName, config.type ?? "after", mod, config.key, config.callback);
}
...
export async function wpPatchMany<M extends object, K extends Extract<keyof M, string>>(
    ...
    return modules.map(mod => mod ? runPatch(patcher, pluginName, type, mod as M, key, callback) : null);
}
...
export function wpWaitGetBulk<T extends WaitQueryWithTimeout[]>(...queries: [...T]): Promise<{ [K in keyof T]: any }> {
...
export async function wpWaitGetBulkKeyed<T extends Record<string, WaitQueryWithTimeout>>(
    ...
    const entries = await Promise.all(
        Object.entries(queries).map(
            async ([key, query]) => [key, await resolveWaitQueryWithTimeout(query as WaitQueryWithTimeout)] as const
        )
    );
    return Object.fromEntries(entries) as any;
}
...
function wpWaitCached(key, filter) {
    if (!moduleCache.has(key)) {
        const p = waitForExportBySource(key, filter).then(v => {
            resolvedCache.set(key, v);
            return v;
        });
