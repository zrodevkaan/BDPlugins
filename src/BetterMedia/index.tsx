...
const { Webpack, Patcher, DOM, Components, React, ContextMenu, UI, Net, Data, Plugins } = new BdApi(
  'BetterMedia'
);
...
const MediaModal = Webpack.getByStrings(
...
const openMedia = async (
  ...
  const discordDoesntEncodeWebpsInDiscordNative = shouldReverse
...
const MediaContainer = ({ url: urlA, width, isThirdParty, provider }) => {
  ...
  const buildSearchMenu = () => {
    ...
    const elements = [
      createSubmenuItem('reverse-search', 'Reverse Search', reverseSearchItems, {}, <SearchIcon />),
      ...
      createContextMenuItem('open-settings', 'Open Settings', () => {
        ...
        ModalSystem.openModal(props => (
          <Modal {...props} title="BetterMedia Settings">
            <SettingsPanel />
          </Modal>
        ));
      }),
        ...
        <div className="bm-media-controls" style={{ gap: `${gap}px` }}>
          ...
          <div ref={owoRef}>
            <Popout
              ...
              clickTrap={true}
              renderPopout={props => (
                <InformationPopout provider={provider} url={url} {...props}></InformationPopout>
              )}
            >
              {props => (
...
const makeCircularPfp = (
  url: string,
  borderColor: string = '#7289da',
...
const RotateIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor">
    <path d="M12 6v3l4-4-4-4v3c-5.5 0-10 4.5-10 10 0 2.2.7 4.2 1.9 5.8l1.4-1.4C4.5 16.8 4 14.5 4 12c0-4.4 3.6-8 8-8z" />
    <path d="M20 12c0-4.4-3.6-8-8-8v1.8c3.4 0 6.2 2.8 6.2 6.2 0 2.5-1.5 4.7-3.6 5.7l1.4 1.4c2.3-1.3 3.8-3.7 3.8-6.3z" />
  </svg>
);
const FireIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor">
...
const SparklesIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor">
    <path d="m9 11 3-3 3 3-3 3-3-3z" />
    <path d="M18.5 6.5 21 4l-2.5-2.5L16 4l2.5 2.5zM18.5 17.5 21 20l-2.5 2.5L16 20l2.5-2.5zM5.5 6.5 8 4 5.5 1.5 3 4l2.5 2.5zM5.5 17.5 8 20l-2.5 2.5L3 20l2.5-2.5z" />
  </svg>
);
...
const ZapIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor">
    <path d="M13 1 8.5 8.5H1l4.5 6L10 22l4.5-7.5H22l-4.5-6L13 1z" />
  </svg>
);
...
export default class BetterMedia {
    ...
    Patcher.before(
      ...
      (_, args) => {
        if (
          ...
          args[0].location === 'ChannelAttachmentUpload' ||
          DataStore.settings.enabledGallery == false
        ) {
          return;
        }
        ...
        DataStore.settings.reverseModalGallery
          ? mediaItems.push(selectedItem)
          : mediaItems.unshift(selectedItem);
        ...
        return {
          ...
          items: [
            ...
            },
            {
              type: 'button',
              label: 'Open Sticker',
              action: () => openMedia(owo, false, undefined, false),
            },
            {
              type: 'submenu',
              id: 'reverse-search',
  ...
  AUCM(res, props) {
    ...
    const isAnimatedGuildBanner =
      guildMemberProfile?.banner && guildMemberProfile.banner.startsWith('a_');

    const animatedNormalImg = isAnimatedAvatar
      ? normalImg.replace('.png', '.gif').replace('.webp', '.gif')
      : null;
    const animatedNormalBanner = isAnimatedBanner
      ? normalBanner.replace('.png', '.gif').replace('.webp', '.gif')
      : null;
    const animatedGuildImg =
      isAnimatedAvatar && guildImg
        ? guildImg.replace('.png', '.gif').replace('.webp', '.gif')
        : null;
    const animatedGuildBanner =
      isAnimatedGuildBanner && guildBanner
        ? guildBanner.replace('.png', '.gif').replace('.webp', '.gif')
        : null;
    ...
    if (normalImg) {
      ...
      DataStore.settings.canvasFeatures &&
        advancedItems.push({
          type: 'submenu',
