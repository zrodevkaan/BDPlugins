...
var { Webpack, Webpack: { Filters }, ContextMenu, DOM, React, Components, UI } = BdApi;
var { useState, useId, useRef, useLayoutEffect, useMemo, useReducer, useEffect } = React;
var [
  VoiceIcon,
  ModalRoot,
  openModal,
...
var Section = Webpack.getByStrings("cancelAnimationFrame(", "text-xs/semibold", ".useReducedMotion)");
var snowflakeUtils = Webpack.getByKeys("extractTimestamp");
var quantize = Webpack.getMangled("[[0,0,0]]", {
...
var OpenImageModal = Webpack.getByStrings(".shouldHideMediaOptions", "hasMediaOptions:", "numMediaItems:", { searchExports: true });
var GuildMemberCountStore = Webpack.getStore("GuildMemberCountStore");
var ChannelMemberStore = Webpack.getStore("ChannelMemberStore");
var GuildMemberStore = Webpack.getStore("GuildMemberStore");
var GuildStore = Webpack.getStore("GuildStore");
var PermissionStore = Webpack.getStore("PermissionStore");
...
var GetAudioCDN = Webpack.getByStrings(`}=window.GLOBAL_ENV;return`);
var StickersStore = Webpack.getStore("StickersStore");
var Sounds = Webpack.getStore("SoundboardStore");
var UserModal = Webpack.getByKeys("openUserProfileModal");
var getDefaultAvatar = (id) => Number(BigInt(id) >> 22n) % 6;
var { copyImage } = Webpack.getModule((x) => x.copyImage);
var Endpoints = Webpack.getModule(Filters.byKeys("GUILD_EMOJI", "GUILD_EMOJIS"), { searchExports: true });
var PermissionsBits = Webpack.getModule(Filters.byKeys("MANAGE_GUILD_EXPRESSIONS"), { searchExports: true });
var HTTP = Webpack.getModule((m) => typeof m === "object" && m.del && m.put, { searchExports: true });
...
var markdownWrapper = Webpack.getByKeys("parse", "defaultRules", "parseTopic");
...
function AboutTab({ guild }) {
  return React.createElement(
    ...
    guild.vanityURLCode && React.createElement(
      Section,
      { heading: "Vanity URL" },
...
function StickersTab({ guild }) {
  ...
  return React.createElement(
    ...
    !filteredStickers.length ? React.createElement(EmptyState) : React.createElement(
      "div",
      { className: "bd-gp-emoji-grid-emojis" },
      filteredStickers.map(
        (sticker) => React.createElement(
          "div",
          {
            key: sticker.id,
            ...
            onContextMenu: (e) => {
              ContextMenu.open(e, ContextMenu.buildMenu([
                ...
                },
                {
                  ...
                  action: () => {
                    openModal(
                      (props) => React.createElement(GuildSelector, {
                        data: { ...sticker, type: 2 },
                        props
                      })
                    );
...
function EmojiTab({ guild }) {
  ...
  return React.createElement(
    ...
    !filteredEmojis.length ? React.createElement(EmptyState) : React.createElement(
      "div",
      { className: "bd-gp-emoji-grid-emojis" },
      filteredEmojis.map(
...
function FriendsTab({ guild }) {
  ...
  return React.createElement(
    ...
    ids.length ? ids.map(
      (userId) => React.createElement(UserItem, { key: userId, userId, guildId: guild.id })
        ...
        React.createElement(
          "div",
          {
            ...
            onContextMenu: (event2) => {
              ContextMenu.open(event2, ContextMenu.buildMenu([
                { label: "Copy Link", action: () => copy(icon) }
              ]));
            }
          },
          icon ? React.createElement("img", { src: icon, height: 120, width: 120 }) : React.createElement("div", null, guild.acronym)
        )
      ),
      React.createElement(
        "div",
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            tab === Tabs.ABOUT ? React.createElement(AboutTab, { guild }) : tab === Tabs.FRIENDS ? React.createElement(FriendsTab, { guild }) : tab === Tabs.EMOJIS ? React.createElement(EmojiTab, { guild }) : tab == Tabs.SOUNDS ? React.createElement(SoundsTab, { guild }) : tab == Tabs.STICKERS ? React.createElement(StickersTab, { guild }) : tab == Tabs.IGNORE ? React.createElement(IgnoredTab, { guild }) : React.createElement(BlockedTab, { guild })
          )
        )
      )
    )
...
function openGuildProfileModal(guildId) {
  openModal(
    (props) => React.createElement(GuildProfile, { guildId, ...props }),
