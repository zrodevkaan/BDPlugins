...
// src/LastOnline/index.tsx
var index_exports = {};
__export(index_exports, {
  default: () => LastOnline
});
module.exports = __toCommonJS(index_exports);
var { Patcher, Webpack, Utils, React, Data, UI, Components } = new BdApi("LastOnline");
var DMUser = Webpack.getByStrings("activities", "isMultiUserDM", "isMobile", { defaultExport: false });
var ActivityClasses = Webpack.getByKeys("activityStatusText", "fullWidth");
var ChannelStore = Webpack.getStore("ChannelStore");
var SystemUI = Components;
...
var LastOnlineData = class {
  ...
  handleStatusChange(userId, oldStatus, newStatus) {
    ...
    if (!user) return;
    if (oldStatus === "offline" && newStatus !== "offline" || oldStatus !== "offline" && newStatus === "offline") {
  ...
  shouldTrackUser(userId) {
    ...
    const excludedUsers = Array.isArray(this.settings.excludeUsers) ? this.settings.excludeUsers.map((id) => id.trim()) : this.settings.excludeUsers ? this.settings.excludeUsers.split(",").map((id) => id.trim()) : [];
  ...
  getLastOnline(userId) {
    ...
    if (this.settings.showOnlyIfRecent) {
      const recentThreshold = (/* @__PURE__ */ new Date()).getTime() - this.settings.recentActivityThreshold * 24 * 60 * 60 * 1e3;
    ...
    const formattedLastOnline = data.lastOnline ? this.formatTimestamp(data.lastOnline.timestamp) : null;
    const formattedLastMessage = data.lastMessage ? this.formatTimestamp(data.lastMessage.timestamp) : null;
    return {
      lastOnline: {
        ...data.lastOnline,
  ...
  cleanupOldData() {
    ...
    const cutoffTimestamp = (/* @__PURE__ */ new Date()).getTime() - maxTrackAge * 24 * 60 * 60 * 1e3;
...
var DataClass = new LastOnlineData();
function getSettingById(settingId) {
  const settings = Data.load("settings") || DEFAULT_SETTINGS;
  return settings[settingId] !== void 0 ? settings[settingId] : DEFAULT_SETTINGS[settingId];
}
var LastSeenComponent = ({ userId }) => {
  ...
  const construct = tooltipFormat.replace("{lastOnline}", lastOnlineText).replace("{lastMessage}", lastMessageText);
      ...
      res.type = React.useMemo(() => {
        ...
        const newType = function(props) {
          ...
          return React.cloneElement(ret, {
            children() {
              const res2 = ret.props.children.apply(this, arguments);
