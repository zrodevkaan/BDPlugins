...
class LastOnline {
    ...
    start() {
        ...
        this.yeah = Patcher.after(DMUser, 'ZP', (_, args, res) => {

            const originalType = res.type;
            res.type = function (props) {
                const result = originalType.call(this, props);

                if (!result?.props?.children?.props?.children) return result;

                const originalChildren = result.props.children.props.children;
                result.props.children.props.children = function (...args) {
                    const childResult = originalChildren.apply(this, args);

                    const userObj = Utils.findInTree(
                        childResult,
